...
@Component({
  ...
  template: `
    <aside class="sidebar" [class.collapsed]="collapsed" [class.dark]="isDarkMode$ | async"
      ...
      <div class="sidebar-toggle" (click)="toggle()"
           ...
           role="button" [attr.aria-expanded]="!collapsed" tabindex="0"
           (keydown.enter)="toggle()" (keydown.space)="$event.preventDefault(); toggle()">
      ...
      <div class="sidebar-sections">
        <ng-container *ngFor="let section of (filteredSections$ | async)">
          ...
          <div class="section-divider" *ngIf="collapsed" role="separator"></div>
          <a *ngFor="let item of section.items"
            ...
            <span class="nav-badge" *ngIf="item.route === '/purchase-requests' && (pendingRequestsCount$ | async) as count">{{ count }}</span>
          </a>
        </ng-container>
      </div>
    </aside>
  `,
  styles: [`
    :host { display: block; position: sticky; top: 64px; height: calc(100vh - 64px); align-self: flex-start; z-index: 90; }
    .sidebar { width: 240px; height: 100%; background: linear-gradient(135deg, var(--c-dark, #052659) 0%, var(--c-mid, #5483B3) 100%); border-right: 1px solid rgba(255, 255, 255, 0.1); display: flex; flex-direction: column; padding: 12px 0; transition: width 0.3s cubic-bezier(0.4, 0, 0.2, 1), background 0.3s ease; position: relative; overflow: hidden; flex-shrink: 0; box-shadow: 2px 0 16px rgba(0,0,0,0.2); }
    .sidebar.collapsed { width: 64px; }
    .sidebar.dark { box-shadow: 2px 0 16px rgba(0,0,0,0.5); }
    .sidebar-toggle { display: flex; align-items: center; justify-content: center; width: 28px; height: 28px; border-radius: 8px; background: rgba(255, 255, 255, 0.15); cursor: pointer; margin: 0 auto 12px; transition: transform, opacity, color, background-color, border-color, box-shadow 0.2s ease; flex-shrink: 0; }
    ...
    .nav-item { display: flex; align-items: center; gap: 12px; padding: 10px 14px; border-radius: 10px; text-decoration: none; color: rgba(255, 255, 255, 0.85); font-size: 14px; font-weight: 500; cursor: pointer; transition: transform, opacity, color, background-color, border-color, box-shadow 0.2s ease; white-space: nowrap; position: relative; overflow: hidden; }
    ...
    .nav-badge { background: #ef4444; color: white; font-size: 11px; font-weight: 700; padding: 2px 6px; border-radius: 10px; margin-left: auto; line-height: 1; box-shadow: 0 2px 4px rgba(239,68,68,0.3); }
    .sidebar.collapsed .nav-label { opacity: 0; width: 0; overflow: hidden; }
    .sidebar.collapsed .nav-badge { position: absolute; top: 6px; right: 6px; padding: 2px 4px; font-size: 9px; }
...
export class SidebarComponent implements OnInit {
  ...
  constructor(private themeService: ThemeService, private authService: AuthService, private apiService: ApiService) {
  ...
  ngOnInit() {
    ...
    this.pendingRequestsCount$ = this.authService.currentUserRole$.pipe(
      switchMap(role => {
        if (role === 'Accountant') {
          return this.apiService.getPurchaseRequests().pipe(
