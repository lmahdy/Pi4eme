  ...
  template: `
    <div class="csv-upload" (dragover)="onDragOver($event)" (dragleave)="onDragLeave($event)" (drop)="onDrop($event)" [class.dragging]="dragging">
      <div class="upload-area">
        ...
        <p class="upload-text">{{ 'CSV_UPLOAD.DRAG_DROP' | translate }} <strong>.csv</strong> {{ 'CSV_UPLOAD.FILE_HERE' | translate }}</p>
        <p class="upload-sub">{{ 'CSV_UPLOAD.OR' | translate }}</p>
        <label class="btn-browse">
          {{ 'CSV_UPLOAD.BROWSE' | translate }}
