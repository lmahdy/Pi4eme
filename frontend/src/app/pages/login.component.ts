...
@Component({
      ...
      <div class="login-card">
        ...
        <form *ngIf="mode === 'password' && step === 'credentials'" (ngSubmit)="submit()">
          ...
          <button class="btn-primary" type="submit" [disabled]="loading">
            {{ loading ? ('LOGIN.SIGNING_IN' | translate) : ('LOGIN.SUBMIT' | translate) }}
          </button>
        </form>
        ...
        <div *ngIf="step === 'otp'" class="otp-section">
          ...
          <h3>{{ 'LOGIN.OTP_TITLE' | translate }}</h3>
          <p class="otp-hint">{{ 'LOGIN.OTP_HINT' | translate }}</p>
          ...
          <button class="btn-primary" (click)="submitOtp()" [disabled]="loading || otpCode.length !== 6">
            {{ loading ? ('LOGIN.OTP_VERIFYING' | translate) : ('LOGIN.OTP_VERIFY' | translate) }}
          </button>

          <p *ngIf="otpError" class="error-msg">{{ otpError }}</p>
          <button class="btn-back" (click)="backToLogin()">← {{ 'LOGIN.BACK' | translate }}</button>
        </div>

        <!-- ─────────── FACE LOGIN MODE ─────────── -->
        <div *ngIf="mode === 'face' && step === 'credentials'" class="face-section">
          <div class="form-group">
            <label>{{ 'LOGIN.YOUR_EMAIL' | translate }}</label>
          ...
          <div class="camera-container" [class.hidden]="!cameraActive">
            ...
            <div class="camera-overlay">{{ 'LOGIN.LOOK_CAMERA' | translate }}</div>
          </div>

          <div class="status-badge" [ngClass]="faceStatus">
            <span *ngIf="faceStatus === 'idle'">{{ 'LOGIN.STATUS_IDLE' | translate }}</span>
            <span *ngIf="faceStatus === 'loading'">{{ 'LOGIN.STATUS_LOADING' | translate }}</span>
            <span *ngIf="faceStatus === 'camera'">{{ 'LOGIN.STATUS_READY' | translate }}</span>
            <span *ngIf="faceStatus === 'scanning'">{{ 'LOGIN.STATUS_SCANNING' | translate }}</span>
            <span *ngIf="faceStatus === 'success'">{{ 'LOGIN.STATUS_SUCCESS' | translate }}</span>
            <span *ngIf="faceStatus === 'failed'">{{ errorMsg }}</span>
            <span *ngIf="faceStatus === 'no-face'">{{ 'LOGIN.STATUS_NO_FACE' | translate }}</span>
          </div>
        ...
        <div class="divider" *ngIf="step === 'credentials' && mode === 'password'">{{ 'LOGIN.OR' | translate }}</div>
  ...
  styles: [`
    ...
    .tabs {
      display: flex; gap: 0;
      border: 1.5px solid var(--c-input-border); border-radius: 10px;
    ...
    .tab:hover:not(.active) { background: var(--c-hover-bg); color: var(--c-text); }
    ...
    .hint { font-size: 13px; color: var(--c-text-muted); margin-top: 14px; text-align: center; }
    ...
    .otp-section h3 { margin: 0; font-size: 20px; font-weight: 700; color: var(--c-text); border: none !important; padding: 0 !important; }
    .otp-hint { margin: 0; font-size: 13px; color: var(--c-text-muted); text-align: center; }
    .otp-input {
      width: 170px; text-align: center; font-size: 30px; letter-spacing: 8px;
      padding: 14px; border-radius: 10px; border: 2px solid var(--c-mid);
    ...
    .status-badge {
      ...
      background: var(--c-input-bg); color: var(--c-text); font-size: 13px;
    ...
    .divider {
      ...
      margin: 14px 0 6px; color: var(--c-text-muted); font-size: 12px; font-weight: 600;
    ...
    @media (max-width: 480px) {
      ...
      .otp-input { width: 100%; font-size: 24px; letter-spacing: 4px; }
