  ...
  template: `
    <div class="login-wrapper">
      <div class="login-card">
        ...
        <form *ngIf="mode === 'password' && step === 'credentials'" (ngSubmit)="submit()" aria-label="Connexion par mot de passe" novalidate>
          <div class="form-group">
            <label for="login-email">{{ 'AUTH.EMAIL' | translate }}</label>
            <input
              id="login-email"
              type="email"
              [(ngModel)]="email"
          ...
          <button class="btn-primary" type="submit" [disabled]="loading" [attr.aria-busy]="loading">
            {{ loading ? ('SETTINGS.SAVING' | translate) : ('AUTH.SIGNIN_BTN' | translate) }}
          </button>
        </form>

        <!-- ─────────── 2FA OTP STEP ─────────── -->
        <section *ngIf="step === 'otp'" class="otp-section" aria-label="Vérification en deux étapes" aria-live="polite">
          <div class="otp-icon" aria-hidden="true">🔐</div>
          <h3>{{ 'SETTINGS.TWO_FACTOR' | translate }}</h3>
          <p class="otp-hint" id="otp-hint">{{ 'SETTINGS.ENTER_CODE' | translate }}</p>
          ...
          <input
            ...
            [attr.aria-label]="'SETTINGS.ENTER_CODE' | translate"
          />

          <button class="btn-primary" (click)="submitOtp()" [disabled]="loading || otpCode.length !== 6" [attr.aria-busy]="loading">
            {{ loading ? ('SETTINGS.ACTIVATING' | translate) : ('SETTINGS.ACTIVATE_2FA' | translate) }}
          </button>

          <p *ngIf="otpError" id="otp-error" class="error-msg" role="alert">{{ otpError }}</p>
          <button class="btn-back" (click)="backToLogin()" aria-label="Retour à la connexion">← {{ 'SETTINGS.BACK' | translate }}</button>
        </section>

        <!-- ─────────── FACE LOGIN MODE ─────────── -->
        <div *ngIf="mode === 'face' && step === 'credentials'" class="face-section">
          <div class="form-group">
            <label for="face-email">{{ 'AUTH.EMAIL' | translate }}</label>
            <input
              id="face-email"
              type="email"
              [(ngModel)]="email"
          ...
          <div class="camera-container" [class.hidden]="!cameraActive">
            <video autoplay playsinline muted class="camera-preview" aria-label="Caméra de reconnaissance faciale"></video>
            <div class="camera-overlay" aria-hidden="true">{{ 'FACE.CAMERA_OVERLAY' | translate }}</div>
          </div>

          <div class="status-badge" [ngClass]="faceStatus" role="status" aria-live="polite">
            <span *ngIf="faceStatus === 'idle'">{{ 'AUTH.FACE_IDLE' | translate }}</span>
            <span *ngIf="faceStatus === 'loading'"><span aria-hidden="true">⏳</span> {{ 'FACE.LOADING_MODELS' | translate }}</span>
            <span *ngIf="faceStatus === 'camera'"><span aria-hidden="true">✅</span> {{ 'AUTH.FACE_READY' | translate }}</span>
            <span *ngIf="faceStatus === 'scanning'"><span aria-hidden="true">🔍</span> {{ 'FACE.SCANNING' | translate }}</span>
            <span *ngIf="faceStatus === 'success'"><span aria-hidden="true">✅</span> {{ 'AUTH.FACE_SUCCESS' | translate }}</span>
