...
@Component({
  ...
  template: `
    ...
    <div class="grid grid-2" *ngIf="kpis && kpis.count > 0">
      <div class="card"><h3>{{ 'SALES.REVENUE_TIME' | translate }}</h3><div class="chart-wrapper"><canvas baseChart [data]="lineChartData" type="line" [options]="lineChartOptions" role="img" [attr.aria-label]="'SALES.REVENUE_TIME' | translate"></canvas></div></div>
      <div class="card"><h3>{{ 'SALES.REVENUE_PRODUCT' | translate }}</h3><div class="chart-wrapper"><canvas baseChart [data]="barChartData" type="bar" [options]="barChartOptions" role="img" [attr.aria-label]="'SALES.REVENUE_PRODUCT' | translate"></canvas></div></div>
    </div>
    <div class="grid grid-2" *ngIf="kpis && kpis.count > 0">
      <div class="card"><h3>{{ 'SALES.REVENUE_CUSTOMER' | translate }}</h3><div class="chart-wrapper doughnut-wrapper"><canvas baseChart [data]="doughnutData" type="doughnut" [options]="doughnutOptions" role="img" [attr.aria-label]="'SALES.REVENUE_CUSTOMER' | translate"></canvas></div></div>
      <div class="card" *ngIf="sales.length > 0">
        <h3>{{ 'SALES.RECENT' | translate }}</h3>
        <div class="table-scroll">
          <table class="table" aria-label="Ventes récentes">
            <caption class="visually-hidden">{{ 'SALES.RECENT' | translate }}</caption>
            <thead><tr>
              <th scope="col">{{ 'COMMON.DATE' | translate }}</th>
              <th scope="col">{{ 'ACCOUNTANT.TABLE_EMPLOYEE' | translate }}</th>
              <th scope="col">{{ 'COMMON.ITEM' | translate }}</th>
              <th scope="col">{{ 'PURCHASES.ALERTS' | translate }}</th>
              <th scope="col">{{ 'COMMON.QTY' | translate }}</th>
              <th scope="col">{{ 'COMMON.UNIT_PRICE' | translate }}</th>
            ...
            <tbody>
              <tr *ngFor="let row of sales">
                ...
                <td><button class="btn-del" (click)="deleteSale(row._id)" [attr.aria-label]="'Supprimer la vente ' + row.product">&#x2715;</button></td>
              </tr>
            </tbody>
          </table>
        </div>
  ...
  styles: [`
    ...
    .perf-card { background: var(--bg-primary, white); border-radius: 12px; padding: 16px; border: 1px solid var(--border-color, #e2e8f0); transition: transform, opacity, color, background-color, border-color, box-shadow 0.2s; }
