...
@Component({
  ...
  template: `
    ...
    <div class="search-bar" role="search">
      ...
      <input type="text" id="supplier-search" [(ngModel)]="searchQuery" (input)="onSearch()" [placeholder]="'PARTNERS.SEARCH_SUPPLIERS' | translate" class="search-input" [attr.aria-label]="'PARTNERS.SEARCH_SUPPLIERS' | translate" />
    </div>
      ...
      >
        ...
        >
          ...
          <div class="partner-card-actions">
            <button class="btn-action edit" (click)="editSupplier(s)" [attr.aria-label]="('SETTINGS.EDIT' | translate) + ' ' + s.name" [title]="'SETTINGS.EDIT' | translate">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
                <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" />
                <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" />
              </svg>
            </button>
            <button class="btn-action delete" (click)="deleteSupplier(s._id)" [attr.aria-label]="('ADMIN.DELETE' | translate) + ' ' + s.name" [title]="'ADMIN.DELETE' | translate">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
    ...
    <div class="modal-overlay" *ngIf="showModal" (click)="showModal = false" role="dialog" aria-modal="true" [attr.aria-label]="editingSupplier ? ('PARTNERS.EDIT_SUPPLIER' | translate) : ('PARTNERS.ADD_SUPPLIER' | translate)">
      <div class="modal-card" (click)="$event.stopPropagation()">
        <div class="modal-header">
          <h2 id="supplier-modal-title">{{ editingSupplier ? ('PARTNERS.EDIT_SUPPLIER' | translate) : ('PARTNERS.ADD_SUPPLIER' | translate) }}</h2>
          <button class="btn-close" (click)="showModal = false" aria-label="Fermer">&#x00D7;</button>
        </div>
        <form (ngSubmit)="saveSupplier()" class="modal-form" aria-labelledby="supplier-modal-title">
          <div class="form-group">
            <label for="supplier-name">{{ 'PARTNERS.NAME' | translate }} *</label>
            <input id="supplier-name" type="text" [(ngModel)]="form.name" name="name" required aria-required="true" [placeholder]="'PARTNERS.NAME' | translate" />
          </div>
          <div class="form-group">
            <label for="supplier-email">{{ 'PARTNERS.EMAIL' | translate }}</label>
            <input id="supplier-email" type="email" [(ngModel)]="form.email" name="email" [placeholder]="'PARTNERS.EMAIL' | translate" autocomplete="email" />
          </div>
          <div class="form-group">
            <label for="supplier-phone">{{ 'PARTNERS.PHONE' | translate }}</label>
            <input id="supplier-phone" type="text" [(ngModel)]="form.phone" name="phone" [placeholder]="'PARTNERS.PHONE' | translate" autocomplete="tel" />
          </div>
