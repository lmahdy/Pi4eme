...
@Component({
  ...
  template: `
    ...
    <div class="search-bar" role="search">
      ...
      <input type="text" id="customer-search" [(ngModel)]="searchQuery" (input)="onSearch()" [placeholder]="'PARTNERS.SEARCH_CUSTOMERS' | translate" class="search-input" [attr.aria-label]="'PARTNERS.SEARCH_CUSTOMERS' | translate" />
    </div>
      ...
      >
        ...
        >
          ...
          <div class="partner-card-actions">
            <button class="btn-action edit" (click)="editCustomer(c)" [attr.aria-label]="('SETTINGS.EDIT' | translate) + ' ' + c.name" [title]="'SETTINGS.EDIT' | translate">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
                <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" />
                <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" />
              </svg>
            </button>
            <button class="btn-action delete" (click)="deleteCustomer(c._id)" [attr.aria-label]="('ADMIN.DELETE' | translate) + ' ' + c.name" [title]="'ADMIN.DELETE' | translate">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
    ...
    <div class="modal-overlay" *ngIf="showModal" (click)="showModal = false" role="dialog" aria-modal="true" [attr.aria-label]="editingCustomer ? ('PARTNERS.EDIT_CUSTOMER' | translate) : ('PARTNERS.ADD_CUSTOMER' | translate)">
      <div class="modal-card" (click)="$event.stopPropagation()">
        <div class="modal-header">
          <h2 id="customer-modal-title">{{ editingCustomer ? ('PARTNERS.EDIT_CUSTOMER' | translate) : ('PARTNERS.ADD_CUSTOMER' | translate) }}</h2>
          <button class="btn-close" (click)="showModal = false" aria-label="Fermer">&#x00D7;</button>
        </div>
        <form (ngSubmit)="saveCustomer()" class="modal-form" aria-labelledby="customer-modal-title">
          <div class="form-group">
            <label for="customer-name">{{ 'PARTNERS.NAME' | translate }} *</label>
            <input id="customer-name" type="text" [(ngModel)]="form.name" name="name" required aria-required="true" [placeholder]="'PARTNERS.NAME' | translate" autocomplete="name" />
          </div>
          <div class="form-group">
            <label for="customer-email">{{ 'PARTNERS.EMAIL' | translate }}</label>
            <input id="customer-email" type="email" [(ngModel)]="form.email" name="email" [placeholder]="'PARTNERS.EMAIL' | translate" autocomplete="email" />
          </div>
          <div class="form-group">
            <label for="customer-phone">{{ 'PARTNERS.PHONE' | translate }}</label>
            <input id="customer-phone" type="text" [(ngModel)]="form.phone" name="phone" [placeholder]="'PARTNERS.PHONE' | translate" autocomplete="tel" />
          </div>
          <div *ngIf="formMsg" class="status-msg" [class.error]="formError" role="alert">{{ formMsg }}</div>
          <button type="submit" class="btn-submit" [disabled]="!form.name.trim() || saving" [attr.aria-busy]="saving">
  ...
  styles: [`
    ...
    .btn-add {
      ...
      transition: transform, opacity, color, background-color, border-color, box-shadow 0.2s ease;
    }
    ...
    .btn-drag-handle {
      ...
      cursor: grab; flex-shrink: 0; transition: transform, opacity, color, background-color, border-color, box-shadow 0.2s ease;
    }
    ...
    .btn-submit {
      ...
      transition: transform, opacity, color, background-color, border-color, box-shadow 0.2s; box-shadow: 0 2px 10px rgba(5,38,89,0.2);
    }
